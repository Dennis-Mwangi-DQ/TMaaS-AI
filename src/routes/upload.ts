...
uploadRouter.post('/', upload.single('document'), async (req, res) => {
  try {
    ...
    const rawText = await loadDocument(file.buffer, file.mimetype, file.originalname);
    
    // Extract evidence in the background or await it
    // For prototype, we await it to immediately show evidence
    const evidence = await extractEvidence(rawText, sessionId, file.originalname);

    await updateSession(sessionId, {
      documentsUploaded: [...session.documentsUploaded, file.originalname]
    });

    res.json({ sessionId, evidenceSummary: `${evidence.length} evidence records extracted` });
  } catch (error) {
